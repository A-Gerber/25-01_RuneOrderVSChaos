...
public abstract class UserSkill
{
    ...
    public UserSkill(Sprite iconOnButton, ParticleSystem effect, AudioClip audioClip, int manaCost)
    {
        ...
        _iconOnButton = iconOnButton != null ? iconOnButton : throw new ArgumentNullException("iconOnButton is null", nameof(iconOnButton));
        _effect = effect != null ? effect : throw new ArgumentNullException("effect is null", nameof(effect));
        AudioClip = audioClip != null ? audioClip : throw new ArgumentNullException("audioClip is null", nameof(audioClip));
    ...
    internal List<LocalPosition> GetSkillCoordinates(LocalPosition position, int minBorderArea, int maxBorderArea)
    {
        ...
        for (int i = 0; i < Configuration.GetLength(0); i++)
        {
            for (int j = 0; j < Configuration.GetLength(1); j++)
            {
                ...
                {
                    int coordinateX = position.X + j + OffsetX;
                    int coordinateZ = position.Z + i + OffsetZ;
    ...
    internal virtual void PlayEffects(Vector3 position)
    {
        _effect.Play();
        Used?.Invoke();
    }
